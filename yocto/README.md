...
**Gitignored:** `build-rpi/`, `meta-device-base/files/rauc-keys/*`, and
`meta-device-base/recipes-core/rauc/files/*.pem` / `*.srl` (see
[`meta-device-base/.gitignore`](meta-device-base/.gitignore)).
`recipes-core/rauc/files/system.conf` is **tracked** in git.
...
#### `recipes-core/rauc/files/system.conf` (in git)

Shipped in the layer at
[`meta-device-base/recipes-core/rauc/files/system.conf`](meta-device-base/recipes-core/rauc/files/system.conf).
The `rauc-conf` bbappend installs it on the gateway as `/etc/rauc/system.conf`.
It defines A/B rootfs slots (`/dev/mmcblk0p2` / `p3`, U-Boot names `A` / `B`),
dm-verity bundles, and `/data/rauc` for update state — aligned with
`wic/device-base-dual.wks.in`. Edit and commit it in git when you change the
partition layout or `compatible` string; only `*.pem` and `*.srl` in that
